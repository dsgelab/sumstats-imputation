...
def z_meta( studies : List[Tuple['Study','VariantData']], is_het_test = False):
    ...
    # Z = sum(w_i * Z_i) / sqrt( sum(w_i^2) ), w_i = sqrt(effective_size)
    ...
    for s in studies:
        ...
        z.append(math.sqrt(study.effective_size) * dat.z) # vector Zi*wi, not used later
        weights.append(math.sqrt(study.effective_size))
        effs_size_org.append(dat.z)

        sum_weights += study.effective_size
        sum_z += math.sqrt(study.effective_size) * dat.z
        tot_size += study.effective_size

    z_meta = sum_z / math.sqrt(sum_weights)
    if is_het_test:
        het_p = het_test(effs_size_org, weights, z_meta)
    else:
        het_p = None
...
def check_eff_field(field):
    if field.lower() in ["z"]:
        ...
        raise Exception("test statistic must be z")
...
class VariantData:
    ...
    def __init__(self, chr, pos, ref, alt, z, pval, extra_cols=[]):
        self.chr = chr
    ...
    def equalize_to(self, other:'VariantData') -> bool:
        """
            ...
            If it is, changes this variant's alleles and z accordingly
        ...
        if (self.chr == other.chr and self.pos == other.pos):
            ...
            if is_symmetric( other.ref, other.alt ):
                ...
                if self.ref == other.ref and self.alt == other.alt:
                    ...
                    self.z = -1 * self.z if self.z is not None else None
                    t = self.alt
                    self.alt = self.ref
                    self.ref = t
                    return True
    ...
    def __str__(self):
        return "chr:{} pos:{} ref:{} alt:{} z:{} pval:{} ".format(self.chr, self.pos, self.ref, self.alt, self.z, self.pval)
...
def run():
    ...
    with open( outfile, 'w' ) as out:
        ...
        for oth in studs[1:len(studs)]:
            out.write( "\t" +  "\t".join( [ oth.name + "_Z", oth.name + "_pval"] ))
            out.write( ("\t" if len(oth.extra_cols) else "") + "\t".join( [oth.name + "_" + c for c in oth.extra_cols] ) )

            if args.pairwise_with_first:
                for m in methods:
                    out.write("\t" + studs[0].name + "_" + oth.name + "_" + m + "_meta_Z\t" + studs[0].name + "_" + oth.name + "_" +  m + "_meta_p")

        out.write("\tall_meta_N")
        for m in methods:
            if args.is_het_test:
                out.write("\tall_"+m+"_meta_Z\tall_"+  m +"_meta_p\tall_"+ m +"_het_p")
            else:
                out.write("\tall_"+m+"_meta_Z\tall_"+  m +"_meta_p")

        if args.leave_one_out:
            for s in studs:
                out.write("\t" + "leave_" + s.name + "_N")
                for m in methods:
                    out.write( "\t" +  "\t".join( ["leave_" + s.name + "_" + m + "_meta_Z", "leave_" + s.name + "_" + m + "_meta_p", "leave_" + s.name + "_" + m + "_meta_het_p"] ))
        ...
        while len(matching_studies)>0:
            ...
            for i,_ in enumerate(studs):
                if next_var[i] is not None:
                    outdat.extend([format_num(next_var[i].z), format_num(next_var[i].pval, 3) ])
                    ...
                    if next_var[0] is not None:
                        ...
                        for m in met:
                            ...
                            outdat.append(format_num(m[1], 3))
                    else:
            ...
            if args.leave_one_out:
                for s,_ in enumerate(studs):
                    ...
                    if len(matching_studies_loo) > 1:
                        ...
                        for m in met:
                            outdat.extend([format_num(num) for num in m[0:1]])
                            outdat.extend([format_num(num, 3) for num in m[1:3]])
                    elif len(matching_studies_loo) == 1:
                        outdat.extend( [format_num(matching_studies_loo[0][1].z), format_num(matching_studies_loo[0][1].pval, 3), 'NA']  * len(methods) )
                    else:
