...
option_list = list(
  make_option(c("--original_sstat"), type="character", default=NULL,
              help="original sumstat", metavar="character"),
  make_option(c("--imputed_sstat"), type="character", default=NULL,
              help="imputed sumstat", metavar="character"),
  make_option(c("--out"), type="character", default = NULL,
              help="output file name ", metavar="character"),
  make_option(c("--N_cases"), type="integer", default = NULL,
              help="number of cases "),
  make_option(c("--N_controls"), type="integer", default = NULL,
...
ori <- ori %>%
  mutate(Z = BETA/SE,
         ...
         raiss.imputed = 0) %>%
  select(rsID, '#CHR' = CHR, POS, Allele1, Allele2, Z, p.value, raiss.imputed, imputationInfo, BETA, SE, AF_Allele2, N)
...
out$N_cases <- opt$options$N_cases
out$N_controls <- opt$options$N_controls
out <- out %>%
  arrange(`#CHR`, POS)

oo <- opt$options$out
fwrite(out, oo, sep = '\t', na = "NA", quote = FALSE)