task run_range {
    ...
    command <<<
        ...
        /META_ANALYSIS/scripts/meta_analysis_z.py ${opts} --chrom ${chrom} ${conf} "${pheno}_${method}_chr${chrom}_meta" ${method} && \
        echo "`date` done"
        ...
        with gzip.open('${file}', 'rt') as f:
            ...
            for i,h in enumerate(header.split('\t')):
                ...
                if h.endswith('_N') and not h.endswith('_meta_N'):
                    n_idx.append(i)
                if h.endswith('_N_cases'):
                    ca_idx.append(i)
                if h.endswith('_N_controls'):
                    ct_idx.append(i)
                if h.endswith('_raiss.imputed'):
                    r_idx.append(i)

            if len(n_idx) != len(ca_idx):
            ...
            print(header + '\tall_meta_imputed\tall_meta_sample_N\tall_meta_N_cases\tall_meta_N_controls\trsid')
            for line in f:
                line = line.strip()
                s = line.split('\t')
                chr = int(s[h_idx['#CHR']])
                ...
                #        af_total = af_total + float(s[idx]) * int(s[n_idx[i]])
                        ...
                        ca_sum = ca_sum + int(s[ca_idx[i]])
                        ct_sum = ct_sum + int(s[ct_idx[i]])
                        r_sum = r_sum + int(s[r_idx[i]])

                #af_total = af_total / n_sum

                #print(line + '\t' + str(n_sum) + '\t' + numpy.format_float_scientific(af_total, precision=3) + '\t' + rsid)
                print(line + '\t' + str(r_sum) + '\t' + str(n_sum) + '\t' + str(ca_sum) + '\t' + str(ct_sum) + '\t' + rsid)
        EOF
...
task filter_cols {
    ...
    command <<<
        ...
        gunzip -c ${file} | awk '
        BEGIN {FS=OFS="\t"}
        NR==1 {for(i=1;i<=NF;i++) { a[$i]=i; if (i<6||$i~"_AF_Allele2$"||$i~"_AF_fc$"||$i~"_raiss.imputed$"||$i~"^all_"||$i~"^rsid") use[i]=1 }}
...
task plot {
    ...
    command <<<
        ...
        Rscript /sumstats_imputation/scripts/qqplot_sumstats_imp.R --file ${base} \
        --bp_col "POS" --chrcol "#CHR" --pval_col "all_${method}_meta_p" --snp_col "rsID" \
...
workflow run_meta {
    ...
    call filter_variants {
        ...
        call plot {
       input: sumstat=filter_cols.out, method=method
    }
}
